...
class LunaApiSurfaceTest {
    ...
    fun exposesALargeDiscordJsStyleSurface() {
        ...
        listOf("user", "Client", "permissions", "Intents", "events", "storage", "rules", "hosts", "vpn", "ui", "fs", "debug", "schema", "Collection", "List", "Set", "Store", "FormBuilder", "sdk", "systems", "kit", "Machine", "Dashboard", "forge", "Schedule", "Ruleset", "string", "table", "json", "Events", "PageBuilder", "EmbedBuilder", "RuleBuilder", "HostsBuilder", "REST").forEach { name ->
        ...
        assertThat(luna.get("ui").get("fold").isfunction()).isTrue()
        assertThat(luna.get("ui").get("steps").isfunction()).isTrue()
        assertThat(luna.get("features").get("forge").toboolean()).isTrue()
        assertThat(luna.get("ui").get("textarea").isfunction()).isTrue()
        assertThat(luna.get("ui").get("reload").isfunction()).isTrue()
        assertThat(luna.get("fs").get("read").isfunction()).isTrue()
        assertThat(luna.get("API_LEVEL").toint()).isEqualTo(2)
        assertThat(luna.get("Events").get("SettingChanged").tojstring()).isEqualTo("settingChanged")
        val vars = LuaTable()
        vars.set("name", "Luna")
        assertThat(luna.get("i18n").get("t").call(LuaValue.valueOf("hi"), LuaValue.valueOf("Hello {name}"), vars).tojstring())
            .isEqualTo("Hello Luna")
        assertThat(luna.get("features").get("assets").toboolean()).isTrue()
        assertThat(luna.get("features").get("sdk").toboolean()).isTrue()
        assertThat(luna.get("debug").get("inspect").isfunction()).isTrue()
        val spec = LuaTable()
        spec.set("host", "string")
        val row = LuaTable()
        row.set("host", "example.com")
        assertThat(luna.get("schema").get("check").call(row, spec).toboolean()).isTrue()
        val dumped = luna.get("debug").get("inspect").call(row).tojstring()
        assertThat(dumped).contains("example.com")
        ...
        val list = luna.get("List").get("of").call(LuaValue.valueOf("a"), LuaValue.valueOf("b"))
        assertThat(list.get("size").call().toint()).isEqualTo(2)
        assertThat(luna.get("string").get("snake").call(LuaValue.valueOf("HelloWorld")).tojstring()).isEqualTo("hello_world")
    }
